    ...
    > SARSA<E>
{
    ...
    fn sarsa_update(
        ...
        (s, a, r, s_, a_): (&S, &AC, f64, &S, Option<&AC>),
    ) -> f64 {
        let prev_index = Self::tabular_index(s, a);
        let cur = match a_ {
            Some(action) => {
                let cur_index = Self::tabular_index(s_, action);
    ...
    > PolicyEstimator for SARSA<E>
{
    ...
    fn policy_search(
            ...
            && episode < self.episode_limit
        {
                ...
                if temporal_difference.len() < 2 {
                    ...
                    match (first, last) {
                        (
                            ...
                            episode_variation += self.sarsa_update(
                                agent,
                                &mut action_value,
                                (observation, action, *reward, obs2, Some(act2)),
                            );
                        }
            ...
            let final_state = environment.final_observation(agent);
